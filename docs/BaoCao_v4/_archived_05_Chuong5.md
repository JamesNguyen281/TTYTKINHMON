...
Công tác kiểm thử được tiến hành nhằm đánh giá chất lượng website trước khi bàn giao. Toàn bộ hoạt động kiểm thử được tổ chức xoay quanh Playwright kết hợp TypeScript, bổ sung một số kịch bản thủ công cho các luồng phức tạp chưa được tự động hóa. Bám sát nội dung đề cương đã được phê duyệt, công tác kiểm thử tập trung vào ba loại theo chuẩn ISTQB: kiểm thử giao diện, kiểm thử chức năng và kiểm thử hồi quy.

Kiểm thử giao diện (UI Testing) tập trung đánh giá phần hiển thị tới người dùng. Các yếu tố được kiểm tra gồm vị trí của các phần tử trên layout, kích thước nút bấm trên thiết bị di động đạt tối thiểu 44 px theo Apple HIG, tình trạng tràn ngang ở viewport 320 px và kích thước font input tối thiểu 16 px để tránh iOS tự zoom khi gõ. Bộ kiểm thử được chạy trên bốn viewport gồm desktop 1366, desktop 1920, mobile iPhone X và mobile iPhone SE. Kết quả thu được phản ánh độ tương thích của giao diện trên nhiều kích thước màn hình khác nhau.

Kiểm thử chức năng (Functional Testing) xác minh các nghiệp vụ chính hoạt động đúng yêu cầu. Luồng đặt lịch phải sinh `Appointment` ở trạng thái `pending`, thao tác xác nhận của lễ tân phải sinh booking code đúng định dạng `KMyymmddS###`, bác sĩ A không được phép mở hồ sơ chẩn đoán bệnh nhân của bác sĩ B. Bên cạnh việc kiểm tra phản hồi giao diện, các bản ghi trong CSDL và bảng `audit_system` cũng được kiểm tra trực tiếp để bảo đảm dữ liệu được lưu đầy đủ. Cách tiếp cận này giúp phát hiện các sai lệch giữa hành vi hiển thị và trạng thái dữ liệu thực sự.

Kiểm thử hồi quy (Regression Testing) được áp dụng sau mỗi lần thay đổi mã nguồn, đặc biệt khi tái cấu trúc service hoặc thay đổi schema. Toàn bộ bộ test được chạy lại để phát hiện các lỗi cũ tái xuất hiện. Để bảo đảm tính lặp lại, mỗi test được thiết kế độc lập, tự seed dữ liệu trước khi chạy và dọn dẹp sau khi kết thúc, không phụ thuộc thứ tự thực thi. Đây là điều kiện cần để bộ kiểm thử có thể được chạy trên môi trường tích hợp liên tục.

Ngoài ba loại trên, đồ án duy trì thêm một bộ smoke test ngắn bằng Postman cho tầng HTTP đã trình bày ở mục 1.4.6, kèm theo một số kịch bản thủ công nhằm đánh giá trải nghiệm thực tế khi triển khai public. Tổng hợp kết quả được trình bày ở mục 5.3.
...
## 5.1. Kiểm thử bằng Playwright

### 5.1.1. Kiểm thử trang công khai
...
Bảng 5.3. Test case Đăng ký tài khoản mới
...
### 5.1.2. Kiểm thử Cổng Lễ tân

#### a. Kiểm thử Xác nhận lịch hẹn

Mã test case: TC_REC_001

Bảng 5.4. Test case Xác nhận lịch hẹn ở trạng thái Pending
...
Bảng 5.5. Test case Tìm lịch theo số điện thoại
...
Bảng 5.6. Test case Check-in bệnh nhân
...
### 5.1.3. Kiểm thử Cổng Bác sĩ

#### a. Kiểm thử Chẩn đoán

Mã test case: TC_DOC_001

Bảng 5.7. Test case Lập hồ sơ chẩn đoán
...
Bảng 5.8. Test case Bác sĩ A không xem được bệnh nhân của BS B
...
### 5.1.4. Kiểm thử mobile responsive

#### a. Kiểm thử overflow ngang trên iPhone X (375 px)

Mã test case: TC_MOB_001

Bảng 5.9. Test case Kiểm tra overflow ngang trên iPhone X
...
Bảng 5.10. Test case Kiểm tra tap target ≥ 44 px trên iPhone SE
...
## 5.2. Kiểm thử thủ công

Một số kịch bản nghiệp vụ phức tạp khó tự động hóa hoàn toàn được kiểm thử thủ công bởi sinh viên thực hiện:

### 5.2.1. Kiểm thử thủ công luồng đặt lịch — duyệt — khám trọn vòng

Bảng 5.11. Test case kiểm thử end-to-end thủ công
...
### 5.2.2. Kiểm thử bằng Postman cho các HTTP endpoint
...
Bảng 5.12. Một số request kiểm thử trong Postman collection
...
Toàn bộ collection được lưu thành tệp `TTYTKM.postman_collection.json` đặt trong thư mục `postman/` của repository. Có hai phương án để chạy lại bộ kiểm thử. Phương án thứ nhất là mở ứng dụng Postman và dùng chức năng "Import" để nạp tệp collection. Phương án thứ hai là sử dụng dòng lệnh `newman run TTYTKM.postman_collection.json`. Newman là CLI runner đi kèm Postman, phù hợp khi tích hợp vào pipeline tự động hoặc khi không cần đến giao diện đồ họa.

![Hình 5.1. Báo cáo HTML từ Newman — chạy 8 request, 12 assertion, 0 fail trong 829 ms](images/hinh-4-4.png){width=16cm}

### 5.2.3. Kiểm thử triển khai public qua Cloudflare Tunnel
...
Bảng 5.13. Test case kiểm thử triển khai public
...
Bảng 5.14. Tổng hợp kết quả kiểm thử toàn hệ thống
...
![Hình 5.2. Báo cáo HTML Playwright Test Runner — toàn bộ 264 test pass, 15 skip, 0 fail](images/hinh-4-3.png){width=16cm}

### 5.3.1. So sánh thời gian thực thi

Bảng 5.15. So sánh thời gian kiểm thử thủ công và kiểm thử tự động
...
### 5.3.3. Kết luận chương 5

Bộ kịch bản kiểm thử tự động bằng Playwright bao quát ba loại kiểm thử theo định hướng đề cương gồm UI Testing, Functional Testing và Regression Testing trên các nghiệp vụ chính của website TTYT phường Kinh Môn. Việc kết hợp Playwright cho E2E tự động, Postman cho smoke test HTTP endpoint và kiểm thử thủ công cho acceptance kèm deploy public đã giúp phát hiện và khắc phục 18 lỗi trong quá trình phát triển, bao gồm 2 lỗi critical, 9 lỗi high, 5 lỗi medium và 2 lỗi low. So với cách kiểm thử thủ công, Playwright rút ngắn thời gian kiểm thử khoảng 13 lần, phù hợp với các dự án có nhiều vai trò người dùng và workflow phức tạp như hệ thống y tế.
